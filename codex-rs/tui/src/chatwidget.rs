...
impl ChatWidget {
    ...
    /// Forwards the active-agent summary into the bottom-pane footer/status pipeline.
    pub(crate) fn set_active_agent_summary(
        &mut self,
        summary: Option<crate::bottom_pane::ActiveAgentStatusSummary>,
    ) {
    ...
    fn handle_thread_item(
        ...
        match item {
            ThreadItem::UserMessage { content, .. } => {
                if let Some(cell) = content.first().and_then(|item| match item {
                    UserInput::Text { text, .. } if content.len() == 1 => {
                        history_cell::new_subagent_notification_event(text)
                    }
                    _ => None,
                }) {
                    self.add_to_history(cell);
