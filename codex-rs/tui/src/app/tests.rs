...
async fn active_agent_status_summary_tracks_displayed_thread_label() {
    let mut app = make_test_app().await;
    let primary = ThreadId::from_string("00000000-0000-0000-0000-000000000100").unwrap();
    let child_a = ThreadId::from_string("00000000-0000-0000-0000-000000000101").unwrap();
    let child_b = ThreadId::from_string("00000000-0000-0000-0000-000000000102").unwrap();

    app.primary_thread_id = Some(primary);
    app.active_thread_id = Some(primary);
    app.upsert_agent_picker_thread(
        primary, /*agent_nickname*/ None, /*agent_role*/ None, /*is_closed*/ false,
    );
    app.upsert_agent_picker_thread(
        child_a,
        Some("Robie".to_string()),
    ...
    assert!(
        app.chat_widget
            .status_line_text()
            .as_deref()
            .is_some_and(|status| status.starts_with("Main [default] • 2 open agents"))
    );

    app.active_thread_id = Some(child_a);
    ...
    assert!(
        app.chat_widget
            .status_line_text()
            .as_deref()
            .is_some_and(|status| status.starts_with("Robie [explorer] • 2 open agents"))
    );
}
...
async fn agent_start_and_stop_refresh_open_agent_count_in_status_summary() {
    let mut app = make_test_app().await;
    let primary = ThreadId::from_string("00000000-0000-0000-0000-000000000110").unwrap();
    let child_a = ThreadId::from_string("00000000-0000-0000-0000-000000000111").unwrap();
    let child_b = ThreadId::from_string("00000000-0000-0000-0000-000000000112").unwrap();
    ...
    assert!(
        app.chat_widget
            .status_line_text()
            .as_deref()
            .is_some_and(|status| status.starts_with("Main [default] • 1 open agent"))
    );
    ...
    app.mark_agent_picker_thread_closed(child_a);
    assert!(
        app.chat_widget
            .status_line_text()
            .as_deref()
    ...
    app.mark_agent_picker_thread_closed(child_b);
    assert!(
        app.chat_widget
            .status_line_text()
            .as_deref()
            .is_some_and(|status| status.starts_with("Main [default]"))
    );
}

#[tokio::test]
async fn thread_closed_notification_immediately_reduces_open_agent_count() {
    let mut app = make_test_app().await;
    let primary = ThreadId::from_string("00000000-0000-0000-0000-000000000120").unwrap();
    let child_a = ThreadId::from_string("00000000-0000-0000-0000-000000000121").unwrap();
    let child_b = ThreadId::from_string("00000000-0000-0000-0000-000000000122").unwrap();
