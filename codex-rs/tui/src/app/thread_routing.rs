...
impl App {
    ...
    fn active_agent_status_summary(&self) -> Option<crate::bottom_pane::ActiveAgentStatusSummary> {
    ...
    pub(super) fn sync_active_agent_label(&mut self) {
        let summary = self.active_agent_status_summary();
        let label = summary.as_ref().map(|summary| summary.label.clone());
        self.chat_widget.set_active_agent_label(label);
        self.chat_widget.set_active_agent_summary(summary);
