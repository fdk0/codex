...
#[test]
fn subagent_notification_event_formats_completion_detail() {
    let cell = new_subagent_notification_event(
        r#"<subagent_notification>{"agent_path":"/root/dispatcher","status":{"completed":"done\nnext step"}}</subagent_notification>"#,
    )
    .expect("subagent notification should parse");
