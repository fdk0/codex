...
const HOOK_CONTEXT_MAX_DISPLAY_ROWS: usize = 5;
...
fn hook_context_preview_lines(text: &str, width: u16) -> Vec<Line<'static>> {
    ...
    push_wrapped_hook_context_line(
        ...
        width,
        Line::from(prefix),
        Line::from(continuation_prefix.clone()),
    );
    for line in source_lines {
        if line.is_empty() {
            wrapped.push(continuation_prefix.clone().into());
        } else {
            push_wrapped_hook_context_line(
                &mut wrapped,
                line,
...
mod tests {
    ...
    fn completed_hook_long_single_line_context_is_truncated_only_in_display() {
        ...
        assert_eq!(display.len(), HOOK_CONTEXT_MAX_DISPLAY_ROWS + 1);
        assert_eq!(
    ...
    fn completed_hook_non_context_entries_are_not_truncated() {
        for kind in [
            ...
            assert!(
                display.iter().any(|line| line.trim_start() == "fifth"),
