...
pub(crate) async fn run(
    ...
    let input_json = match serde_json::to_string(&AfterCompactionCommandInput::from_context(
        CommandInputContext {
...
mod tests {
    ...
    fn handler(matcher: Option<&str>, profile: Option<&str>) -> ConfiguredHandler {
        ConfiguredHandler {
            event_name: HookEventName::AfterCompaction,
    ...
    fn request(source: AfterCompactionSource, profile: Option<&str>) -> AfterCompactionRequest {
        AfterCompactionRequest {
            session_id: codex_protocol::ThreadId::new(),
            turn_id: "turn-1".to_string(),
            cwd: PathBuf::from("/tmp"),
            transcript_path: None,
            active_profile: profile.map(str::to_owned),
