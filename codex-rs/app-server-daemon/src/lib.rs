...
pub async fn ensure_remote_control_started() -> Result<RemoteControlStartOutput> {
    ...
    Daemon::from_environment()?
        .ensure_remote_control_started_with_client_name(None)
        .await
}
...
pub async fn ensure_remote_control_ready() -> Result<RemoteControlReadyOutput> {
    ensure_supported_platform()?;
    Daemon::from_environment()?
        .ensure_remote_control_ready_with_client_name(None)
        .await
}

pub async fn ensure_remote_control_ready_with_client_name(
    client_name: Option<String>,
) -> Result<RemoteControlReadyOutput> {
    ensure_supported_platform()?;
    Daemon::from_environment()?
        .ensure_remote_control_ready_with_client_name(client_name)
        .await
}
...
impl Daemon {
    ...
    async fn ensure_remote_control_started_with_client_name(
        &self,
        client_name: Option<String>,
        ...
        let output = self
            .bootstrap_locked(BootstrapOptions {
                ...
                remote_control_client_name: client_name,
            })
            .await?;
        Ok(RemoteControlStartOutput::Bootstrap(output))
    }

    async fn ensure_remote_control_ready_with_client_name(
        &self,
        client_name: Option<String>,
    ) -> Result<RemoteControlReadyOutput> {
        let daemon = self
            .ensure_remote_control_started_with_client_name(client_name)
            .await?;
        let remote_control =
    ...
    async fn set_remote_control(
        ...
        let _operation_lock = self.acquire_operation_lock().await?;
        self.set_remote_control_locked(options).await
    }
