...
async fn multi_agent_v2_close_agent_releases_capacity_for_next_spawn() {
    let (_, default_turn) = make_session_and_context().await;
    let mut config = (*default_turn.config).clone();
    let _ = config.features.enable(Feature::MultiAgentV2);
    ...
    let root = manager
        ...
        .await
        .expect("root thread should start");
    let session = root.thread.session.clone();
    ...
    let Err(err) = result else {
        panic!("second spawn_agent should hit the active execution cap");
    };
    let FunctionCallError::RespondToModel(message) = err else {
...
async fn multi_agent_v2_followup_task_completion_notifies_parent_on_every_turn() {
    ...
    .unwrap_or_else(|err| {
        panic!(
            "parent should receive one completion notification per child turn: {err:?}; ops={:#?}",
            manager.captured_ops()
        )
    });
...
async fn spawn_agent_env_overrides_extend_runtime_shell_environment_policy() {
    ...
    assert_eq!(
        child_turn.config.permissions.shell_environment_policy.r#set,
        HashMap::from([
...
async fn spawn_agent_rejects_non_string_env_values() {
    ...
    let result = SpawnAgentHandler::default()
        ...
        .await;
    let Err(err) = result else {
        panic!("non-string env values should be rejected");
    };
    let FunctionCallError::RespondToModel(message) = err else {
