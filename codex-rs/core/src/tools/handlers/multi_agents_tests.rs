...
async fn wait_for_inter_agent_followup_message_in_history(
    thread: &Arc<CodexThread>,
    expected: &InterAgentCommunication,
) {
    timeout(Duration::from_secs(5), async {
        loop {
            ...
            if saw_envelope {
                assert!(
                    !saw_user_message,
                    "inter-agent followup should stay in its structured instruction envelope, not be flattened to a plain user message"
                );
                break;
            }
    ...
    .await
    .expect("inter-agent followup message should appear in history");
}
...
async fn multi_agent_v2_send_message_interrupts_busy_child_without_triggering_turn() {
    ...
    timeout(Duration::from_secs(5), async {
        loop {
            if !thread.codex.session.has_pending_mailbox_items().await {
                tokio::time::sleep(Duration::from_millis(10)).await;
                continue;
            }
            let history_items = thread
            ...
            if saw_envelope || saw_user_message {
                panic!(
                    "send_message should stay queued in the mailbox without materializing history"
                );
            } else {
                break;
            }
    ...
    .await
    .expect("interrupting v2 send_message should queue mailbox mail without a turn");
...
async fn multi_agent_v2_followup_task_interrupts_busy_child_without_losing_message() {
    ...
    wait_for_inter_agent_followup_message_in_history(
        &thread,
        &InterAgentCommunication::new(
            AgentPath::root(),
            worker_path,
