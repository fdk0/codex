...
use crate::tools::handlers::multi_agents_common::thread_spawn_source as v2_thread_spawn_source;
...
pub(crate) struct CompactParentContextHandler;
...
impl ToolHandler for SpawnAgentHandler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<Self::Output, FunctionCallError> {
        ...
        spawn::handle(session, turn, call_id, function_arguments(payload)?).await
    }
}
...
impl ToolHandler for SendInputHandler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<Self::Output, FunctionCallError> {
        ...
        send_input::handle(session, turn, call_id, function_arguments(payload)?).await
    }
}
...
impl ToolHandler for ResumeAgentHandler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<Self::Output, FunctionCallError> {
        ...
        resume_agent::handle(session, turn, call_id, function_arguments(payload)?).await
    }
}

#[async_trait]
impl ToolHandler for CompactParentContextHandler {
    type Output = FunctionToolOutput;
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<Self::Output, FunctionCallError> {
        ...
        compact_parent_context::handle(session, turn, call_id, function_arguments(payload)?).await
    }
}
...
impl ToolHandler for ListAgentsHandler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<Self::Output, FunctionCallError> {
        ...
        list_agents::handle(session, turn, call_id, function_arguments(payload)?).await
    }
}
...
impl ToolHandler for WaitHandler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<Self::Output, FunctionCallError> {
        ...
        wait::handle(session, turn, call_id, function_arguments(payload)?).await
    }
}
...
impl ToolHandler for CloseAgentHandler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<Self::Output, FunctionCallError> {
        ...
        close_agent::handle(session, turn, call_id, function_arguments(payload)?).await
    }
}
...
mod spawn {
    ...
    pub async fn handle(
        ...
        let spawn_source = v2_thread_spawn_source(
        ...
        let (new_agent_path, new_agent_nickname, new_agent_role) = match new_thread_id {
            Some(thread_id) => {
                let (nickname, role) = session
                    ...
                    .await
                    .unwrap_or((None, None));
                let path = agent_snapshot.as_ref().and_then(|snapshot| {
...
mod send_input {
    ...
    pub async fn handle(
        ...
        let receiver_thread_id = match args.id.as_deref().map(str::trim) {
            Some(id) if !id.is_empty() && !matches!(id, "parent" | "root") => {
                resolve_agent_target(&session, &turn, id).await?
            }
            _ => session.parent_thread_id().await.ok_or_else(|| {
                FunctionCallError::RespondToModel(
                    "send_input requires a target when no parent agent is available".to_string(),
                )
            })?,
        };
...
pub(crate) mod wait {
    ...
    pub async fn handle(
        ...
        if args.ids.is_empty() {
            return Err(FunctionCallError::RespondToModel(
                "targets must be non-empty".to_owned(),
            ));
        }
        let mut requested_thread_ids = Vec::with_capacity(args.ids.len());
        for id in &args.ids {
            requested_thread_ids.push(resolve_agent_target(&session, &turn, id).await?);
        }
...
pub mod close_agent {
    ...
    pub async fn handle(
        ...
        let agent_id = resolve_agent_target(&session, &turn, &args.id).await?;
        let (receiver_agent_nickname, receiver_agent_role) = session
            .services
            .agent_control
...
mod tests {
    ...
    #[tokio::test]
    async fn spawn_agent_with_task_name_returns_task_name_and_send_input_accepts_it() {
        #[derive(Debug, Deserialize)]
        struct SpawnAgentResult {
            agent_id: String,
            task_name: Option<String>,
        ...
        let agent_id = agent_id(&result.agent_id).expect("agent_id should be valid");
        assert_eq!(result.task_name.as_deref(), Some("/root/dispatcher_1"));
        ...
        MultiAgentHandler
            ...
            .await
            .expect("send_input should accept spawned task_name");
    ...
    async fn wait_rejects_wake_enabled_child_agents() {
        ...
        let wait_invocation = invocation(
            ...
            turn,
            "wait",
            function_payload(json!({"targets":[spawn_result.agent_id],"timeout_ms":10000})),
        );
    ...
    fn wait_allows_wake_enabled_child_agents_when_configured() {
        run_async_test_with_large_stack(|| async {
            ...
            let wait_invocation = invocation(
                ...
                turn,
                "wait",
                function_payload(json!({"targets":[spawn_result.agent_id],"timeout_ms":10})),
            );
            let output = MultiAgentHandler
    ...
    #[tokio::test]
    async fn send_input_accepts_target_alias_for_agent_id() {
        let (mut session, turn) = make_session_and_context().await;
        let manager = thread_manager();
        session.services.agent_control = manager.agent_control();
        ...
        MultiAgentHandler
            .handle(invocation)
            .await
            .expect("send_input should accept target alias");
    ...
    #[tokio::test]
    async fn close_agent_accepts_target_alias_for_agent_id() {
        let (mut session, turn) = make_session_and_context().await;
        let manager = thread_manager();
        session.services.agent_control = manager.agent_control();
        let config = turn.config.as_ref().clone();
        ...
        let output = MultiAgentHandler
            .handle(invocation)
            .await
            .expect("close_agent should accept target alias");
        let (content, success) = expect_text_output(output);
        let result: close_agent::CloseAgentResult =
            serde_json::from_str(&content).expect("close_agent result should be json");
        assert_eq!(result.status, status_before);
