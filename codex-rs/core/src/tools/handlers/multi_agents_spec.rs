...
pub fn create_followup_task_tool() -> ToolSpec {
    ...
    ToolSpec::Function(ResponsesApiTool {
        ...
        description: "Send a message to an existing non-root target agent and trigger a turn in that target. Use interrupt=true to redirect work immediately. If interrupt=false and the target is already running, the message is queued and starts the target's next turn after the current turn completes."
            .to_string(),
        strict: false,
        defer_loading: None,
