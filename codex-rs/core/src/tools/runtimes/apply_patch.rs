...
impl ApplyPatchRuntime {
    ...
    fn supports_internal_apply_patch_cli(path: &std::path::Path) -> bool {
        !matches!(
    ...
    fn build_sandbox_command(req: &ApplyPatchRequest) -> Result<SandboxCommand, ToolError> {
...
impl ToolRuntime<ApplyPatchRequest, ExecToolCallOutput> for ApplyPatchRuntime {
    async fn run(
        ...
        #[cfg(not(target_os = "windows"))]
        let command = Self::build_sandbox_command(req)?;
