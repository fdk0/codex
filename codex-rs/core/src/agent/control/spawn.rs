...
async fn latest_child_ids_by_agent_path(
    ...
    for child_thread_id in child_ids {
        let child_metadata = match state
...
impl AgentControl {
    ...
    pub(crate) async fn ensure_v2_agent_loaded(
        ...
        // Older rollouts can reference a role that is no longer configured. Keep the persisted
        // semantic role metadata, but only layer role settings when the current config can resolve
        // them; otherwise resumption must continue with the caller's runtime config.
        if let Some(role_name) = session_source.get_agent_role()
            && resolve_role_config(&config, &role_name).is_some()
        {
    ...
    pub(crate) async fn resume_agent_from_rollout(
        ...
        while let Some((parent_thread_id, parent_depth)) = resume_queue.pop_front() {
            ...
            let child_ids = latest_child_ids_by_agent_path(&state, child_ids).await;
