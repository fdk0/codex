...
#[tokio::test]
async fn on_event_updates_status_from_task_complete_error() {
    let status = agent_status_from_event(&EventMsg::TurnComplete(TurnCompleteEvent {
        turn_id: "turn-1".to_string(),
        started_at: None,
        last_agent_message: Some("partial output".to_string()),
...
async fn spawn_child_completion_notifies_parent_history() {
    ...
    let (parent_thread_id, parent_thread) = harness.start_thread().await;
    let mut child_config = harness.config.clone();
    child_config.agent_wake_parent_on_completion_default = false;
...
async fn leaf_only_routes_descendant_completion_to_direct_parent_only() {
    Box::pin(leaf_only_routes_descendant_completion_to_direct_parent_only_impl()).await;
}

async fn leaf_only_routes_descendant_completion_to_direct_parent_only_impl() {
    let harness = AgentControlHarness::new().await;
    ...
    worker_thread
        ...
        .await;
    worker_thread.session.active_turn.lock().await.take();
    ...
    tester_thread
        ...
        .await;
    tester_thread.session.active_turn.lock().await.take();
...
async fn wake_enabled_child_keeps_parent_lifecycle_pending_until_notified() {
    ...
    child_thread
        ...
        .await;
    child_thread.session.active_turn.lock().await.take();
...
async fn trigger_turn_rearms_wake_subscription_even_when_child_is_running() {
    ...
    child
        ...
        .await;
    child.thread.session.active_turn.lock().await.take();
...
async fn direct_subagent_resume_restores_wake_subscription_for_descendant_followup() {
    ...
    let dispatcher_reconcile_turn = resumed_dispatcher.thread.session.new_default_turn().await;
    resumed_dispatcher
        .thread
        .session
        .send_event(
