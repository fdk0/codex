...
#[tokio::test]
async fn apply_stock_awaiter_override_keeps_builtin_behavior_and_changes_model() {
    let home = TempDir::new().expect("create temp dir");
    tokio::fs::write(
...
"#,
    ...
    apply_role_to_config(&mut config, Some("awaiter"))
        .await
        .expect("awaiter role should apply");

    assert_eq!(config.model.as_deref(), Some("gpt-5.4-mini"));
    assert_eq!(config.model_reasoning_effort, Some(ReasoningEffort::Low));
    assert_eq!(config.background_terminal_max_timeout, 3_600_000);
    assert!(
        config
            .developer_instructions
            .as_deref()
            .is_some_and(|instructions| instructions.contains("You are an awaiter."))
    );
}

#[tokio::test]
async fn apply_stock_explorer_override_keeps_builtin_behavior_and_changes_model() {
    let home = TempDir::new().expect("create temp dir");
    tokio::fs::write(
        home.path().join(CONFIG_TOML_FILE),
        r#"[agents.explorer]
...
fn built_in_config_file_contents_resolves_explorer_only() {
    assert_eq!(
        crate::agent::built_in_roles::config_file_contents(Path::new("missing.toml")),
        None
    );
}
