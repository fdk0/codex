...
impl AgentControl {
    ...
    pub(crate) fn register_session_root(
        ...
        if thread_spawn_parent_thread_id(current_session_source).is_none() {
            self.guards.register_root_thread(current_thread_id);
        }
    }

    pub(crate) fn get_agent_metadata(&self, agent_id: ThreadId) -> Option<AgentMetadata> {
        self.guards.agent_metadata_for_thread(agent_id)
    }
    ...
    pub(crate) async fn resolve_agent_reference(
        ...
        if let Some(thread_id) = self.guards.agent_id_for_path(&agent_path) {
            return Ok(thread_id);
        }
    ...
    fn prepare_thread_spawn(
        ...
        if depth == 1 {
            self.guards.register_root_thread(parent_thread_id);
        }
    ...
    async fn has_active_descendants(
        ...
        let descendants = self.collect_descendants(state, owner_thread_id).await;
...
mod inbox_tests {
    ...
    #[tokio::test]
    async fn compact_parent_for_watchdog_helper_releases_marker_when_parent_is_busy() {
        let harness = AgentControlHarness::new().await;
        let (owner_thread_id, owner_thread) = harness.start_thread().await;
        let watchdog_handle_id = harness
            .control
        ...
        owner_thread
            ...
            .lock()
            .await
            .replace(crate::state::ActiveTurn::default());

        let result = harness
            .control
            .compact_parent_for_watchdog_helper(helper_thread_id)
            .await
            .expect("busy parent should not error");
        assert_eq!(
            result,
        ...
        owner_thread.codex.session.active_turn.lock().await.take();

        let result = harness
            .control
            .compact_parent_for_watchdog_helper(helper_thread_id)
            .await
            .expect("busy result should not leave stale marker");
        match result {
            WatchdogParentCompactionResult::Submitted {
                parent_thread_id, ..
            } => {
            ...
            other => panic!(
                "expected submitted compaction result after busy parent clears, got {other:?}"
            ),
        }
    }
}
