...
impl ShellSnapshot {
    ...
    async fn try_create(
        ...
        Self::try_new_with_env_overrides(codex_home, session_id, session_cwd, shell, state_db, &[])
            .await
    }
    ...
    async fn try_new_with_env_overrides(
        ...
        if let Err(err) = validate_snapshot(shell, &temp_path, session_cwd, env_overrides).await {
