...
#[tokio::test]
async fn load_config_applies_agent_wake_policies() -> std::io::Result<()> {
    let codex_home = TempDir::new()?;
    let cfg = ConfigToml {
...
#[tokio::test]
async fn load_config_defaults_agent_wake_policies_when_omitted() -> std::io::Result<()> {
    let codex_home = TempDir::new()?;
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml::default(),
        ConfigOverrides::default(),
...
fn profile_v2_config_path_resolves_next_to_symlinked_base_config() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        dunce::canonicalize(dotfiles.join("work.config.toml"))?.abs()
    );
    Ok(())
}
...
async fn active_profile_tracks_selected_profile_v2_config() -> anyhow::Result<()> {
    ...
    assert_eq!(config.active_profile_name(), Some("work"));
    Ok(())
}
...
#[tokio::test]
async fn profile_model_auto_compact_token_limit_overrides_root() -> std::io::Result<()> {
    let codex_home = TempDir::new()?;
    let config = load_dispatcher_profile_v2_config(
        &codex_home,
        r#"model = "gpt-5.4"
model_auto_compact_token_limit = 120000
"#,
        r#"model_auto_compact_token_limit = 90000
"#,
        ConfigOverrides::default(),
    )
    .await?;
...
#[tokio::test]
async fn profile_tool_output_token_limit_overrides_root() -> std::io::Result<()> {
    let codex_home = TempDir::new()?;
    let config = load_dispatcher_profile_v2_config(
        &codex_home,
        r#"model = "gpt-5.4"
tool_output_token_limit = 12500
"#,
        r#"tool_output_token_limit = 8000
"#,
        ConfigOverrides::default(),
    )
    .await?;
...
#[tokio::test]
async fn root_limits_apply_when_profile_omits_overrides() -> std::io::Result<()> {
    let codex_home = TempDir::new()?;
    let config = load_dispatcher_profile_v2_config(
        &codex_home,
        r#"model = "gpt-5.4"
...
#[tokio::test]
async fn profile_developer_instructions_override_root() -> std::io::Result<()> {
    let codex_home = TempDir::new()?;
    let config = load_dispatcher_profile_v2_config(
        &codex_home,
        r#"model = "gpt-5.4"
...
"#,
        r#"developer_instructions = "Profile instructions"
"#,
        ConfigOverrides::default(),
    )
    .await?;
...
#[tokio::test]
async fn root_developer_instructions_apply_when_profile_omits_override() -> std::io::Result<()> {
    let codex_home = TempDir::new()?;
    let config = load_dispatcher_profile_v2_config(
        &codex_home,
        r#"model = "gpt-5.4"
...
#[tokio::test]
async fn cli_developer_instructions_override_profile() -> std::io::Result<()> {
    let codex_home = TempDir::new()?;
    let config = load_dispatcher_profile_v2_config(
        &codex_home,
        r#"model = "gpt-5.4"
...
async fn load_dispatcher_profile_v2_config(
    codex_home: &TempDir,
    base_config: &str,
    profile_config: &str,
    ...
    let profile_config_path = codex_home.path().join("dispatcher.config.toml");
    tokio::fs::write(&base_config_path, base_config).await?;
    tokio::fs::write(&profile_config_path, profile_config).await?;

    ConfigBuilder::without_managed_config_for_tests()
        .codex_home(codex_home.path().to_path_buf())
        .loader_overrides(LoaderOverrides {
            user_config_path: Some(profile_config_path.abs()),
            user_config_profile: Some("dispatcher".parse().expect("profile-v2 name")),
            ..LoaderOverrides::without_managed_config_for_tests()
        })
...
"#,
    ...
    let expected_root_usage_hint = append_usage_hint_text(
    ...
    let expected_subagent_usage_hint = append_usage_hint_text(
