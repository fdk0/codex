...
const DEFAULT_MULTI_AGENT_V2_SHARED_USAGE_HINT_TEXT: &str = r#"Note that multi-agent tools cannot be called from inside `functions.exec`. Call `spawn_agent`, `send_message`, `followup_task`, `wait_agent`, `interrupt_agent`, `list_agents`, and `close_agent` only as direct tool calls using the recipient shown in their tool definitions, since they are intentionally absent from the `functions.exec` `tools.*` namespace. Available tools in `functions.exec` are explicitly described with a `tools` namespace in the developer message.
...
pub struct Config {
    ...
    /// Default wake policy used when a spawn_agent call omits wake_parent_on_completion.
    pub agent_wake_parent_on_completion_default: bool,
    /// Behavior when wait_agent targets wake-enabled children.
    pub agent_wait_on_wake_enabled_behavior: AgentWaitOnWakeEnabledBehavior,
    /// Policy for forwarding descendant completion notifications to ancestors.
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let agent_wake_parent_on_completion_default = cfg
            ...
            .unwrap_or(true);
        let agent_wait_on_wake_enabled_behavior =
            cfg.agents
                .as_ref()
