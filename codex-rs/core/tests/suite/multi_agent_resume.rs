...
async fn cold_root_resume_restores_agent_identity_and_role_on_followup() -> Result<()> {
    ...
    .await;
    let _followup_child_request = mount_sse_once_match(
        &server,
        |request: &wiremock::Request| {
            request_has_input_type(request, "agent_message")
    ...
    tokio::time::timeout(Duration::from_secs(5), async {
        loop {
            if server
                ...
                .any(|request| {
                    body_contains(request, FOLLOWUP_TASK)
                        && body_contains(request, ROLE_DEVELOPER_INSTRUCTIONS)
                })
            {
                break;
            }
    ...
    .await
    .expect("timed out waiting for the reloaded worker follow-up request");
