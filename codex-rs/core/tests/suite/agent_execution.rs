...
async fn v2_nested_spawn_checks_shared_active_execution_capacity() -> Result<()> {
    ...
    assert_eq!(
        second_output,
        "agent thread limit reached (max 1); use list_agents to find completed or no-longer-needed agents, then close_agent to release capacity before spawning another agent"
    );
