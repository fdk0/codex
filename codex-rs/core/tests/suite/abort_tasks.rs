...
#[test]
fn interrupt_long_running_tool_emits_turn_aborted() -> anyhow::Result<()> {
...
#[test]
fn interrupt_tool_records_history_entries() -> anyhow::Result<()> {
    run_async_test_with_large_stack(|| async {
        let command = "sleep 60";
...
#[test]
fn interrupt_persists_turn_aborted_marker_in_next_request() -> anyhow::Result<()> {
    run_async_test_with_large_stack(|| async {
        let command = "sleep 60";
