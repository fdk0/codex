...
fn turn_item_contains_subagent_notification(item: &CoreTurnItem) -> bool {
    match item {
...
mod tests {
    ...
    fn subagent_notification_item_events_are_suppressed() {
        let item = CoreTurnItem::UserMessage(codex_protocol::items::UserMessageItem {
        ...
        assert!(turn_item_contains_subagent_notification(&item));
    }
    ...
    -> Result<()> {
        ...
        let OutgoingMessage::AppServerNotification(envelope) = activity_message else {
            bail!("unexpected message: {activity_message:?}");
        };
        let activity = match envelope.notification {
            ServerNotification::ItemCompleted(activity) => activity,
            notification => bail!("unexpected notification: {notification:?}"),
        };
        assert_eq!(
            activity,
            ItemCompletedNotification {
